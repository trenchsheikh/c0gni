...
contract SwarmMemory {
    struct SwarmData {
        bytes32 swarmId;
        address[] memberAgents;
        Pattern[] sharedPatterns;
        PerformanceHistory collectivePerformance;
        ConsensusDecision[] consensusHistory;
        Strategy[] learnedStrategies;
        CoordinationRules coordinationRules;
    }
    ...
    struct Pattern {
        ...
        address discoveredBy;        // Which agent found it
        address[] validatedBy;       // Which agents confirmed it
        uint256 usageCount;
        uint256 chainId;             // Chain where pattern works best
    }
    
    // Collective memory advantage - all patterns shared across chains
    mapping(bytes32 => SwarmData) public swarms;
    mapping(bytes32 => Pattern) public globalPatterns;
}
```
